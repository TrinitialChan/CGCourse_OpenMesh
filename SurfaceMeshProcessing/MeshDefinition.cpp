...
//my function
double MeshTools::AveArea(const Mesh & mesh)
{
	double area = 0.0;
	double num = 0.0;
	for (const auto & f : mesh.faces())
...
double MeshTools::AveCellCircumlength(const Mesh &mesh)
{
	double circum = 0.0;//ÖÜ³¤
	double num = 0.0;
	for (const auto & f : mesh.faces())
	{
		auto heh = mesh.halfedge_handle(f);
		...
		const auto & p2 = mesh.point(mesh.to_vertex_handle(mesh.next_halfedge_handle(heh)));
		num++;
		circum += (p1 - p0).length() + (p2 - p1).length() + (p0 - p2).length();
	}
